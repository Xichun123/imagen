...
These prompt recipes are intended for requests executed through `scripts/imagen.py`.
...
Execution details such as `quality`, `input_fidelity`, masks, output formats, and local output paths are configured with CLI flags documented in `references/cli.md`.
...
Style/medium: vector-like flat logo graphic rendered as raster; flat colors; minimal
...
Style/medium: vector-like flat wordmark rendered as raster; flat colors; minimal
