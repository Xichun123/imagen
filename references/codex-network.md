...
The CLI uses the provider selected from the providers JSON config, so it needs outbound network access to that provider's URL. In many Codex setups, network access is disabled by default and/or the approval policy requires confirmation before networked commands run.
