...
These prompting principles apply to requests executed through `scripts/imagen.py`.

This file covers prompt structure, specificity, and iteration. Execution controls such as `quality`, `input_fidelity`, masks, output format, and output paths are documented in `references/cli.md` and `references/image-api.md`.
...
- See `references/cli.md` and `references/image-api.md` for supported values and examples.
